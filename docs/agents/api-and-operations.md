...
| `scripts/capacity_rehearsal.py`; `scripts/agent_evidence_benchmark.py`; `scripts/storage_history_benchmark.py` | Disposable mixed-workload and storage measurements; strict profiles, resource-free `--plan-only`, receipt conservation, native compression, and verified backup/restore. See [Profile reports and Derivations](../operate/profile-derivations.md#rehearse-capture-alongside-batch-work) |
