...
| `scripts/turn_worktree_observation_spike.py`; `scripts/session_context_retrieval_eval.py` | Local turn-boundary diagnostic (no Fact or network request); isolated pi continuation comparison with private records and enforced transport budgets, respectively. See [Continue a task](../operate/resume-with-evidence.md) |
