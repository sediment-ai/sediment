...
| `sediment_api/deps.py` | explicit ingest/operator authority checks, `/v1/me` identity, `get_store`, body-size middleware, capped body reader, verified-webhook reader |
...
| `scripts/turn_worktree_observation_spike.py` | Disposable local Claude Code turn-boundary diagnostic; emits no Fact or network request |
...
- **The import security gate rejects missing, placeholder, duplicate, or overlapping operator/ingest secrets.** `SEDIMENT_DEV_MODE=true` is the only opt-out. Lesser configuration problems warn (`config.py`).
- Ingest accepts capture or operator authority; query, reports, and Fact reads require operator authority. `/v1/me` accepts either and names `authority` and `client_id`. Unknown credentials return 401; insufficient authority returns 403. Legacy bearer credentials are ingest-only. [ADR 0018](../adr/0018-static-credential-authorities.md) defines revocation and compatibility.
...
- **Every door bounds its body read** at `MAX_BODY_BYTES` (25 MiB, then 413)
  through `BodySizeLimitMiddleware`; `POST /query/evidence/read` uses 64 KiB. It runs pre-auth on every door,
