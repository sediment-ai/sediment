...
| `sediment_api/deps.py` | explicit ingest/operator/retrieval authority and Session-grant checks, `/v1/me` identity, `get_store`, body-size middleware, capped body reader, verified-webhook reader |
...
| `scripts/capacity_rehearsal.py`; `scripts/agent_evidence_benchmark.py`; `scripts/storage_history_benchmark.py`; `scripts/keyword_streaming_benchmark.py` | Disposable workload, growing-Session retrieval, and storage measurements; strict profiles, exact oracle comparisons, receipt conservation, native compression, and backup/restore. See [Profile reports and Derivations](../operate/profile-derivations.md#rehearse-capture-alongside-batch-work) |
...
- **The import security gate rejects missing, placeholder, duplicate, or overlapping operator/ingest secrets.** `SEDIMENT_DEV_MODE=true` is the only opt-out. The optional retrieval token and singleton or plural Session setting always validate, including development mode: strong distinct token, normalized `NonEmptyId` sources, and reserved ingest client name `retrieval`. [ADR 0022](../adr/0022-agent-requested-session-context.md) defines its authority. Lesser problems warn (`config.py`).
- Ingest accepts capture or operator authority; existing query, reports, and Fact reads require operator authority. `/v1/me` also accepts retrieval authority and reports its fixed `source_session_id` or plural `source_session_ids`. Capture enrollment and operator login reject retrieval credentials. Unknown credentials return 401; insufficient authority returns 403. Legacy bearer credentials are ingest-only. [ADR 0018](../adr/0018-static-credential-authorities.md) defines revocation and compatibility.
...
- The lifespan owns one bounded SQLAlchemy engine and engine-backed `FactStore`.
  Workers import report and forge handlers only when those job kinds need them. The API checks the exact Alembic head, then validates runtime privileges in production before starting workers. It never migrates or mutates schema. Ingest borrows its pool; expensive child jobs create separate single-connection engines.
...
- An absent, behind, ahead, unreachable, or unauthorized database blocks startup.
  Diagnostics expose only the operation and sanitized target. Post-start outages return `database_unavailable` 503 after authentication and validation;
  programmer errors return 500.
- **Every door bounds its body read** at `MAX_BODY_BYTES` (25 MiB, then 413)
  through `BodySizeLimitMiddleware`; both exact-fetch routes use 64 KiB and the three keyword context operations use 16 KiB. It runs pre-auth on every door,
  because the server reads envelope bodies before it authenticates them. The
  webhook door keeps its streaming `bytearray` reader and ignores `Content-Length`.
...
- **Commit investigation separates sources.** `/query/commit/{sha}` groups by repository identity with observed names; unknown CI stays in `unresolved_ci_outcomes`, with closed `repository_skipped` source counts. Similarity-selected calls remain `relationship="inferred_call_to_file"` and cannot set factual `attributed` true. One request instant or explicit `as_of` bounds supporting evidence. Supporting reads select exact-commit observations and CI outcomes, metadata for the inferred calls, and decisions from their actual Sessions. Indexed decision-identifier lookup searches visible organization history through the boundary and retains at most two distinct Fact witnesses per requested identifier. It selects no message/raw content, preserves unique-or-drop attachment, and refuses filters over 30,000 keys with `repository_evidence_limit`. The witness set never substitutes for complete bundle identity evidence. Target Attribution selects each repository's earliest eligible owning Push, then reads its Jaccard window plus observed note Sessions' longer window. Captured observation maps, including empty maps, exclude mutable Git notes from historical requests. Repository metadata witnesses preserve complete names/claims and exact queried sources under a separate cap; repeated history stays in PostgreSQL. Earlier Push discovery and scalar SQL grouping retain history costs ([ADR 0024](../adr/0024-targeted-commit-investigations.md)). `sediment commit` forwards name/provider/host/ID and boundary selectors and displays distinct lifetimes.
...
- **The legacy evidence routes require operator authority.** The three `/query/evidence` operations return strict, ASCII-escaped JSON with `Cache-Control: no-store`; request-local snapshots recheck Quarantine. Evidence shares both read slots with queries and reports; a third read receives 503, and reports have no reserved slot. Fixed source and response bounds decline whole requests; [ADR 0021](../adr/0021-bounded-evidence-access.md) defines the contract. `POST /query/context` accepts retrieval or operator authority for the configured Session; it shares that worker limit and emits versioned keyword selection under [ADR 0022](../adr/0022-agent-requested-session-context.md).
- **Discovery never expands authority.** [ADR 0025](../adr/0025-authorized-session-candidate-discovery.md) adds `/query/context/discover` and `/query/context/selected` within an explicit grant of at most 32 Sessions. `SEDIMENT_RETRIEVAL_SESSION_IDS` and the singleton setting are mutually exclusive. Plural mode leaves `/query/context` unavailable. Keyword workers consume bounded streams and encode within the source snapshot; source/state overflow refuses the whole operation. Selected membership is checked before lookup and again in the worker. Complete source limits are aggregate, exact commit anchors never grant access, and changing scope requires a different token and restart.
- **Factual access is independent of usefulness.** The three `/query/context/evidence` operations accept retrieval or operator authority only within the configured Session grant. Inventory, manifest, and exact fetch reuse ADR 0021 projections and limits; parent and worker check membership before storage. Exact fetch reads only selected calls and message sides, preserves repeated occurrences, and can include readable reasoning. `/v1/me` lists the grant; keyword omissions never deny access. Native pi exposes these factual operations independently of discovery mode ([ADR 0026](../adr/0026-grant-scoped-factual-evidence.md)).
...
- **Operational report routes require explicit bounds.** `/v1/reports/model-outcomes` and `/v1/reports/accepted-work-lifecycle` accept a half-open cohort of at most 31 days plus `as_of`. Decision attachment requests at most 30,000 distinct non-null Decision identifiers and searches all visible organization history through `as_of`. Two owners prove ambiguity; excess requested keys return 409 without partial attachment. Report projections omit input/raw and separately refuse output rows over 64 MiB or output populations over 256 MiB before decoding. Attachment reads no message content; its key limit doesn't bound database scan work. They fix the cohort cap at 50,000 Inference calls, await bounded child assembly, cap serialized responses at 64 MiB, return version 1 scope-and-report envelopes, and persist nothing.
...
- Capture installers consume verified ingest enrollment only; explicit `SEDIMENT_INGEST_TOKEN` overrides need a live authority check. Old unclassified profiles require enrollment; no operator fallback exists. `install --codex-profile NAME` writes a private telemetry profile and preserves unrelated settings. `--transcripts` separately enables pi content. Managed Cursor commands pass the generated environment path to `cursor-hook --env-file`; the adapter reads capture exports as data, independent of the desktop environment. Doctor checks this file's endpoint and token presence. `--no-env` leaves process configuration to its owner, including Cursor hooks.
...
- `create_deploy_env.py` creates private deployment credentials without workspace dependencies. Repeat `--ingest-client NAME` to generate distinct client tokens in the initial map; existing files and duplicate or reserved names are refused.
...
- Environment prefixes are `SEDIMENT_`, `SEDIMENT_LABEL_CONFIDENCE_*`, `SEDIMENT_OPENENV_*`, `SEDIMENT_NEMO_GYM_*`, and `SEDIMENT_CAPTURE_DIR`. Settings ignore extra values.
  Compose passes settings only to their consumers. API-only deployment needs neither gateway key; the gateway entrypoint names a missing Anthropic or LiteLLM key. Settings silently ignore unknown variable names.
- Docker uses two `uv sync` layers, a non-root user, and pre-creates `/data/{mirror,export}`. A workspace member needs its own `COPY …/pyproject.toml` line (AGENTS.md's checklist). Compose scopes image tags to `COMPOSE_PROJECT_NAME`; `SEDIMENT_API_PORT` and `SEDIMENT_GATEWAY_PORT` change host ports while retaining loopback bindings. Operator commands depend on PostgreSQL health without starting provisioning. [Docker rehearsal](../operate/validate-deployment.md#verify-the-installed-build) exercises installation and restart in an isolated project.
