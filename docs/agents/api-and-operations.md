...
| `scripts/turn_worktree_observation_spike.py`; `scripts/session_context_retrieval_eval.py` | Local turn-boundary diagnostic (no Fact or network request); isolated pi comparison with frozen tasks, post-run instructed-lookup checks for `shipment-totals`, native tool preflight, private records, and enforced transport budgets, respectively. See [Continue a task](../operate/resume-with-evidence.md) |
