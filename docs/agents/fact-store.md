...
An identified observation requires its exact retained source Push with equal organization and identity. Legacy rows keep absent identities; migration never reconstructs them. `read_session_commit_observations(..., as_of=...)` uses inclusive capture bounds, stable ordering, and optional exact repository-commit or Session filters. It upserts the Session without inventing user identity.
...
Session dossier reads use dedicated `(org_id, session_id, event time, Fact id)` indexes for each Session-scoped Fact table. Delivery joins use `ix_pushes_repo_after` and `ix_ci_repo_commit`. Attribution reads every visible
Push in the org, as `GET /query/commit/{sha}` does, because a commit attributes against the earliest Push that carried it. The dossier selects file-path
presence in SQL and never loads content-bearing columns.
...
`read_context_discovery_source` applies the configured set of at most 32 Session IDs before SQL reads. Its one snapshot shares aggregate 1,000-call, 8 MiB selected-column, and 2,048-part limits across the set. An optional exact identified commit anchor reads only matching observations and their visible identity-consistent source Pushes; it never joins by Push.after_sha or infers a repository grant. [ADR 0023](../adr/0023-authorized-session-candidate-discovery.md) defines the bounded source and absence contract.
...
cap. Read both from the same snapshot before cohort or repository selection.
## Quarantine
