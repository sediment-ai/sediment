...
Session dossier reads use dedicated `(org_id, session_id, event time, Fact id)` indexes for each Session-scoped Fact table. Delivery joins use `ix_pushes_repo_after` and `ix_ci_repo_commit`. Attribution reads every visible Push in the org, as `GET /query/commit/{sha}` does, because a commit attributes against the earliest Push that carried it. The dossier selects file-path presence in SQL and never loads content-bearing columns.
