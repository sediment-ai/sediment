...
Scoped reports and bundle v4 read complete organization-wide `InferenceCallIdentity` projections from that snapshot through `as_of`. Projected `observed_at` retains its instant. Output messages stream for Python alias extraction; input messages and raw payloads aren't selected. The 50,000-row cap and separate 64 MiB output-row ceiling don't bound total decoding cost or database pages. Commit investigation uses the same alias projection and 64 MiB output-row ceiling when keyed decisions require it; its complete snapshot metadata population establishes the identity row bound without adding a fixed population ceiling. No tool-alias index exists; a narrower cohort cannot remove the completeness requirement.
