...
Session dossiers read visible Fact metadata under one read-only, repeatable-read transaction and reject Sessions over the fixed cap. Evidence operations each own a snapshot and refuse inventories over 1,000 visible calls. SQL preflights an 8 MiB sum of selected variable-width columns, counting each row/column once, including metadata. Part reads select only requested calls and message sides and omit raw payloads and user identity. Grant-scoped exact routes reuse these projections without loading or scoring unselected calls. `read_context_source` retains inventory bounds, separately preflights all visible input/output and selected metadata at 8 MiB before transfer, and refuses more than 2,048 canonical parts in that snapshot; [ADR 0021](../adr/0021-bounded-evidence-access.md) defines the separate wire limits.
