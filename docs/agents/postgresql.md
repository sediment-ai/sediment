...
Session dossiers read visible Fact metadata under one read-only, repeatable-read transaction and reject Sessions over the fixed cap. Factual evidence reads refuse inventories over 1,000 visible calls and preflight 8 MiB of selected variable-width columns, counting each row/column once. Part reads select requested calls and sides, omitting raw payloads/user identity. Materialized context readers keep 8 MiB and 2,048-part limits. Production keyword streams preflight 64 MiB total, 8 MiB per row and metadata population, 1,000 calls, then count at most 16,384 parts. `yield_per=1` uses a server cursor; nested contexts own iterator, cursor, and snapshot through encoding. Incomplete normal consumption raises an invariant error. [ADR 0021](../adr/0021-bounded-evidence-access.md) retains separate factual wire limits; [ADR 0022](../adr/0022-agent-requested-session-context.md#selection-and-execution) defines keyword execution.
Bundle v4 reads complete organization-wide `InferenceCallIdentity` projections from that snapshot through `as_of`. Projected `observed_at` retains its instant. Output messages stream for Python alias extraction; input messages and raw payloads aren't selected. The 50,000-row cap and separate 64 MiB output-row ceiling don't bound total decoding cost or database pages. Commit investigation and scoped model/lifecycle report attachment instead use the native hash index over `ARRAY[org_id, call_id]` with exact equality rechecks. The attachment-only reader retains at most two visible Fact witnesses per requested identifier; the 30,000-key budget bounds each operation. Reports request every non-null Decision identifier from the selected Sessions. It reads no output content for attachment. Target Attribution retains separate content limits and selects eligible owner windows before decoding. Commit investigation compacts scalar repository identity/name states in PostgreSQL and retains exact queried sources and visible Push anchors. This bounds transfer by metadata diversity without making SQL grouping independent of history. Push metadata streams without raw or clone content; arbitrary non-head owner discovery can still inspect earlier Git ranges. Complete bundle and unscoped identity consumers retain their population contract ([ADR 0024](../adr/0024-targeted-commit-investigations.md)).
